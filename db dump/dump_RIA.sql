...
CREATE TABLE `address_book` (
  ...
  constraint `ownerUser` foreign key (`owner_id`) references `user` (`id`) on delete cascade on update cascade,
  constraint `contactAccount` foreign key (`contact_account`) references `bank_account` (`id`) on delete cascade on update cascade
